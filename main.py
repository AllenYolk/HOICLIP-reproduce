...
def main(args):
        ...
        if args.with_obj_clip_label and args.with_clip_label:
            param_dicts = [
                ...
                },
                {
                    "params": [
                        p for n, p in model_without_ddp.named_parameters() if (
                            'visual_projection' in n or 'obj_visual_projection' in n or 'clip_model' in n
                        ) and p.requires_grad
                    ],
                    "lr": args.lr_clip,
                },
            ...
            param_dicts = [
                ...
                },
                {
                    "params": [
                        p for n, p in model_without_ddp.named_parameters() if
                        ('visual_projection' in n or 'clip_model' in n) 
                        and p.requires_grad
                    ],
                    "lr": args.lr_clip,
                },
            ...
            param_dicts = [
                {"params": [
                    p for n, p in model_without_ddp.named_parameters() if
                    "backbone" not in n and 'obj_visual_projection' not in n 
                    and 'clip_model' not in n and p.requires_grad
                ]},
                {
                    "params": [
                        p for n, p in model_without_ddp.named_parameters() if
                ...
                },
                {
                    "params": [
                        p for n, p in model_without_ddp.named_parameters() if
                        ('obj_visual_projection' in n or 'clip_model' in n) 
                        and p.requires_grad
                    ],
                    "lr": args.lr_clip,
                },
    ...
    if args.opt_sched == 'multiStep':
        lr_scheduler = MultiStepLRWarmup(
            optimizer, [args.lr_drop], warmup_iter=0, warmup_ratio=0.01,
    ...
    if args.eval:
        ...
        if 'Test result:' not in previous_log:
            ...
            test_stats = evaluate_hoi(
                args.dataset_file, model, postprocessors, data_loader_test,
        ...
        if 'Val result:' not in previous_log:
            ...
            test_stats = evaluate_hoi(
                args.dataset_file, model, postprocessors, data_loader_val,
                args.subject_category_id, device, args
            )
