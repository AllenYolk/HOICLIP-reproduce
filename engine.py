...
@torch.no_grad()
def evaluate_hoi(
    dataset_file, model, postprocessors, data_loader,
    ...
    if args.training_free_enhancement_path:
        ...
        if dataset_file == 'hico':
            evaluator = HICOEvaluator_gen(
                test_pred, gts, data_loader.dataset.rare_triplets,
                data_loader.dataset.non_rare_triplets,
                data_loader.dataset.correct_mat, args=args
            )
        else:
            evaluator = VCOCOEvaluator_gen(
                preds, gts, data_loader.dataset.correct_mat,
                use_nms_filter=args.use_nms_filter
            )
        stats = evaluator.evaluate()

        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            text_hoi_feature = model.module.transformer.hoi_cls
        else:
        ...
        spatial_cls, text_hoi_feature = spatial_cls.to(device), text_hoi_feature.to(device)
        ...
        for a in [1]:
            for co in [1.0]:
                for topk in topk_range:
                    ...
                    ignore_idx = clip_hoi_score_sorted_idx[:, topk:]
                    ...
                    for i in range(len(test_pred)):
                        lo = clip_hoi_score[i].sigmoid() * torch.tensor(co).to(device)
                        test_pred[i]['hoi_scores'] += lo.to("cpu")
                    # testing
                    if dataset_file == 'hico':
                        evaluator = HICOEvaluator_gen(
                            test_pred, gts, data_loader.dataset.rare_triplets,
                        ...
                        evaluator = VCOCOEvaluator_gen(
                            test_pred, gts, data_loader.dataset.correct_mat,
                            use_nms_filter=args.use_nms_filter
                        )
                    stats = evaluator.evaluate()
                    if dataset_file == 'hico':
                    ...
                    if best_performance_1 < re_map:
                        ...
                        with open(os.path.join(root, 'log.txt'), 'a') as f:
                            log = (
                                f'sigmoid after topk: {topk} as: {a} co: {co}' 
                                f'\n performance: {stats}\n'
                            )
                            print(log)
                            f.write(log)
